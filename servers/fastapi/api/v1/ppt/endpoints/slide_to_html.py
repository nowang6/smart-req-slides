...
async def generate_html_from_slide(base64_image: str, media_type: str, xml_content: str, api_key: str, fonts: Optional[List[str]] = None) -> str:
    """
    Generate HTML content from slide image and XML using Moonshot API.
    ...
    """
    print(f"Generating HTML from slide image and XML using Moonshot API...")
    api_key = os.getenv("CUSTOM_LLM_API_KEY")
    try:
        client = OpenAI(base_url="https://api.moonshot.cn/v1", api_key=api_key)

        # Compose input for Moonshot API. Include system prompt, image, OXML and optional fonts text.
        ...
        print(f"Input payload: {messages}")

        print("Making Moonshot API request for HTML generation...")
        ...
        if not html_content:
            raise HTTPException(
                status_code=500,
                detail="No HTML content generated by Moonshot API"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Moonshot API error during HTML generation: {str(e)}"
        )
        ...
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"Moonshot API timeout during HTML generation: {error_msg}"
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"Moonshot API connection error during HTML generation: {error_msg}"
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Moonshot API error during HTML generation: {error_msg}"
            )
...
async def generate_react_component_from_html(html_content: str, api_key: str, image_base64: Optional[str] = None, media_type: Optional[str] = None) -> str:
    """
    Convert HTML content to TSX React component using Moonshot API.
    ...
    try:
        ...
        print("Making Moonshot API request for React component generation...")
        ...
        if not react_content:
            raise HTTPException(
                status_code=500,
                detail="No React component generated by Moonshot API"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Moonshot API error during React generation: {str(e)}"
        )
    except Exception as e:
        # Handle various API errors
        error_msg = str(e)
        ...
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"Moonshot API timeout during React generation: {error_msg}"
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"Moonshot API connection error during React generation: {error_msg}"
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Moonshot API error during React generation: {error_msg}"
            )
...
async def edit_html_with_images(current_ui_base64: str, sketch_base64: Optional[str], media_type: str, html_content: str, prompt: str, api_key: str) -> str:
    """
    Edit HTML content based on one or two images and a text prompt using Moonshot API.
    ...
    try:
        ...
        print("Making Moonshot API request for HTML editing...")
        ...
        if sketch_data_url:
            ...
            content_parts.insert(1, {"type": "image_url", "image_url": {"url": sketch_data_url}})
        ...
        if not edited_html:
            raise HTTPException(
                status_code=500,
                detail="No edited HTML content generated by Moonshot API"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Moonshot API error during HTML editing: {str(e)}"
        )
    except Exception as e:
        # Handle various API errors
        error_msg = str(e)
        ...
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"Moonshot API timeout during HTML editing: {error_msg}"
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"Moonshot API connection error during HTML editing: {error_msg}"
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Moonshot API error during HTML editing: {error_msg}"
            )
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    """
    Convert a slide image and its OXML data to HTML using Moonshot API.
    ...
    try:
        ...
        if not api_key:
            raise HTTPException(
                status_code=500, 
                detail="CUSTOM_LLM_API_KEY environment variable not set"
            )
...
async def edit_html_with_images_endpoint(
    ...
    """
    Edit HTML content based on one or two uploaded images and a text prompt using Moonshot API.
